...
class AgentSwarm:
    def __init__(self):
        ...
        if OPENAI_API_KEY:
            ...
            self.client = AsyncOpenAI(
                api_key=HEURIST_API_KEY, base_url=HEURIST_BASE_URL
            )
        ...
        self.memory = (
            ConversationMemory(
                max_history=MAX_MEMORY_ITEMS, max_age_hours=MEMORY_MAX_AGE_HOURS
            )
            if USE_MEMORY
            else None
        )
    ...
    async def query_agent(
        self, role: Dict, context: str, parameters: Optional[Dict] = None
    ) -> str:
    ...
    async def process_query(
        self, user_query: str, telegram_mode: bool = False, user_id: str = "default"
    ) -> str:
        ...
        triage_response = await self.query_agent(
            AGENT_ROLES["triage"], f"Evaluate this query: '{user_query}'{context_info}"
        )
        ...
        final_response = await self.query_agent(
            AGENT_ROLES["synthesizer"], synthesis_context
        )
    ...
    async def process_query_with_details(
        self, user_query: str, user_id: str = "default"
    ) -> dict:
    ...
    async def process_query_streaming(
        ...
        final_response = ""
        async for chunk in self.query_agent_stream(
            AGENT_ROLES["synthesizer"], synthesis_context, parameters
        ):
    ...
    async def query_agent_stream(
        ...
        try:
            ...
            params = {"messages": messages, "model": self.model, "stream": True}
